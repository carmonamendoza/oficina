...
class XFaeIncomingDoc(models.Model):
    ...
    def action_reload_detail(self):
        ...
        for doc in docs_processed:
            if not doc.lines_ids:
                if doc:
                    detail_doc_vals = fae_utiles.parser_xml_detail(doc.id, 'manual', doc.issuer_xml_doc)
                    for detail_doc in detail_doc_vals:
                        detail_res = self.env['xfae.incoming.documents_det'].sudo().create(detail_doc)

        cron = self.env['ir.cron'].sudo().search([('id', '=', self.env.ref('FAE_app.xfae_reload_incoming_doc_cron_id').id)],
                                          limit=1)
